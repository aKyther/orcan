...
def build_workspace_entry(
    ...
    if meta_raw:
        die(
            ...
            f"{DEFAULT_DEVELOPER_WORKSPACES}/<name> (host meta: .orcan/workspaces/<name>/)"
        )
    meta_path = ensure_dir(
        repo_root / ".orcan" / "workspaces" / ws_name,
...
def prune_stale_workspace_metas(repo_root: Path, active_names: set[str]) -> None:
    """Remove .orcan/workspaces/<name>/ dirs for workspaces no longer in config."""
    host_workspaces = repo_root / ".orcan" / "workspaces"
...
def write_compose_projects(workspaces: list[dict], repo_root: Path) -> str:
    ...
    host_workspaces = ensure_dir(
        repo_root / ".orcan" / "workspaces", "workspace metas root"
    )
    lines = [
...
def synthesize_from_env(project_dir: str, repo_root: Path) -> dict:
    ...
    ws_name = pd.name
    ws_root = default_workspace_root(ws_name, "")
    meta_path = ensure_dir(repo_root / ".orcan" / "workspaces" / ws_name, "workspace meta")
    projects = [
        {
            "name": pd.name,
            "path": str(pd),
