...
- **Config is JSON only** — `orcan.config.json` (no YAML user profiles, no host PyYAML stack)
...
- **Runtime stack** — cockpit (`agent-launcher`: top bar + workspaces/ASSERTIONS \| tmux; **F1** embed disclaimer; **F2** / **`r`** / **`p`** / **`o`**) → tmux 3.6a → zsh; container CMD `orcan-supervisord` (keepalive|ttyd + `context-scan`/`recap`); host `orcan sync --context` for inbox-only compile
