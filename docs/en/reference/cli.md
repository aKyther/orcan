...
| `orcan context tui` | TUI: scan a parent folder, multi-select into a **will add** stack, create/update a workspace; optional one branch → managed worktree per repo (`--sync`, `--yes`). Default scan is **children only** (`--depth 1`; `D` toggles grandchildren). **`l`/`→`** opens a folder; **`u`/`←`** goes up; **`e`** browse; **`h`** recent picks (add without navigating). **Enter** always applies the stack. **Tab** focuses the stack (Space/`x` remove; **Esc** back to list). Tags show `in ws` / `other ws` / conflicts. With an existing config it opens in **manage mode** (`n` → scan to add more); this is what `orcan init` runs by default |
