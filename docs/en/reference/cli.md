...
| `orcan context tui` | TUI: scan a parent folder, multi-select into a **will add** stack, create/update a workspace; optional one branch → managed worktree per repo (`--sync`, `--yes`). Default scan is **children only** (`--depth 1`; `D` toggles grandchildren). **Enter** opens plain folders; **`u`** goes up; **`e`/`h`** jump; picks persist. **Tab** focuses the stack (Space/`x` remove, Enter apply; **Esc** back to list). Tags show `in ws` / `other ws` / conflicts. With an existing config it opens in **manage mode** (`n` → scan to add more); this is what `orcan init` runs by default |
