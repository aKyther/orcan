...
| `orcan context tui` | TUI: scan a parent folder, multi-select repos, create/update a workspace; optional one branch → managed worktree per repo (`--sync`, `--yes`). Default scan is **children only** (`--depth 1`; `d` toggles to grandchildren). **`e`/`h` keep prior picks** across parents. A **will add** stack (side or bottom) previews what Enter writes. With an existing config it opens in **manage mode** instead — rename/move/delete existing workspaces and projects (`n` switches to the scan screen to add more); this is what `orcan init` runs by default |
