...
| `orcan build --agent NAME [...] \| --all-agents [--force] [--no-cache] [--prune]` | Build the standard `orcan:latest` + `orcan:<VERSION>` image with explicit clients (`cursor`, `claude`, `codex`, `gemini`, `copilot`). `/etc/orcan/agents.json` records the choice; `--prune` removes only dangling Orcan images after a successful build — never host-wide Docker or BuildKit cache; never publishes || --all-agents [--force] [--no-cache]` | Build the standard `orcan:latest` + `orcan:<VERSION>` image with explicit clients (`cursor`, `claude`, `codex`, `gemini`, `copilot`). `/etc/orcan/agents.json` records the choice; never publishes |
