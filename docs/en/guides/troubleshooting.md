...
3. Prefix is **C-Space** (not `C-b`). Window jumps: **Alt+1**…**Alt+9** (see [Terminal UI](terminal-ui.md)). In the **cockpit**, **Ctrl/Alt+arrows** focus panes and **Ctrl+Shift+arrows** split — many terminals deliver Alt as Ctrl ([nav mix](terminal-ui.md#cockpit-nav-mix)). Raw `orcan enter --tmux` keeps conf (Ctrl=split, Alt=focus when Meta works).
