...
- Wsadowa, zautomatyzowana Reflection: `orcan-context-reflect`, hook `Stop` **domyślnie włączony**, który batchuje po liczniku tur per sesja (domyślnie 20) zanim wywoła lekki model i zdyspatchuje przez ten sam narzędzie propose. Dosiewany przy pierwszej `orcan sync` dla workspace'u; wyłączenie (i to, że zostaje) przez `orcan context hook disable|enable|status [WORKSPACE ...] [--all]` (`scripts/repository/claude_hook.py`) — merge/usunięcie w `.claude/settings.json` wygenerowanego katalogu głównego workspace'u (odnajdywanego po nazwie przez `workspaces/index.json`), natychmiastowe. Patrz „Wsadowa, zautomatyzowana Reflection" wyżej.
