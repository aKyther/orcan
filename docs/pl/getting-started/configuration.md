...
## Migracja ze starszych instalacji „cind”

Jeśli nadal masz stare dane hosta:

```bash
mv ~/.config/cind ~/.config/orcan
```

Stare lokalne pliki w stylu `cind.config.json` są przestarzałe. Używaj wyłącznie `orcan.config.json`.
