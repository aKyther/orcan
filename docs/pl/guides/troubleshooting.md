...
3. Prefix to **C-Space** (nie `C-b`). Skoki okien: **Alt+1**…**Alt+9** (zobacz [Terminal UI](terminal-ui.md)). W **cockpicie** **Ctrl/Alt+strzałki** = fokus pane, **Ctrl+Shift+strzałki** = split — wiele terminali dostarcza Alt jako Ctrl ([nav mix](terminal-ui.md#cockpit-nav-mix)). Surowy `orcan enter --tmux` zostawia conf (Ctrl=split, Alt=fokus gdy Meta działa).
