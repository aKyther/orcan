...
| `TTYD_PORT` / `TTYD_HOST_PORT` / `TTYD_BIND` / `TTYD_FONT_*` / `TTYD_RENDERER` / `TTYD_THEME` / `TTYD_PING_INTERVAL` | Terminal w przeglądarce (`TTYD_RENDERER`: `webgl` domyślnie, `canvas` jako fallback; `TTYD_BIND` domyślnie `0.0.0.0`; `TTYD_THEME`: `dark`/`navy`, `mocha` albo surowy JSON xterm.js) |
