...
| `orcan context tui` | TUI: skan katalogu-rodzica, multi-select do stosu **will add**, create/update workspace; opcjonalnie jeden branch → managed worktree na każde repo (`--sync`, `--yes`). Domyślny skan to **tylko dzieci** (`--depth 1`; `D` przełącza wnuki). **`l`/`→`** otwiera folder; **`u`/`←`** w górę; **`e`** browse; **`h`** ostatnie picki (dodaj bez chodzenia). **Enter** zawsze zatwierdza stos. **Tab** fokusuje stos (Space/`x` usuwa; **Esc** wraca na listę). Tagi: `in ws` / `other ws` / konflikty. Przy istniejącym configu — **tryb zarządzania** (`n` → skan); to uruchamia domyślnie `orcan init` |
