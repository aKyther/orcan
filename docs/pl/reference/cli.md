...
| `orcan build --agent NAME [...] \| --all-agents [--force] [--no-cache] [--prune]` | Buduje standardowy obraz `orcan:latest` + `orcan:<VERSION>` z jawnym wyborem klientów (`cursor`, `claude`, `codex`, `gemini`, `copilot`). Wybór zapisuje `/etc/orcan/agents.json`; `--prune` po udanym buildzie usuwa tylko osierocone obrazy Orcan — nigdy globalnego cache Docker ani BuildKit; nigdy nie publikuje || --all-agents [--force] [--no-cache]` | Buduje standardowy obraz `orcan:latest` + `orcan:<VERSION>` z jawnym wyborem klientów (`cursor`, `claude`, `codex`, `gemini`, `copilot`). Wybór zapisuje `/etc/orcan/agents.json`; nigdy nie publikuje |
