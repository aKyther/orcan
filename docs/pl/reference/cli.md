...
| `orcan context tui` | TUI: skan katalogu-rodzica, multi-select repo, create/update workspace; opcjonalnie jeden branch → managed worktree na każde repo (`--sync`, `--yes`). Domyślny skan to **tylko dzieci** (`--depth 1`; `d` przełącza na wnuki). **`e`/`h` zachowują wcześniejsze zaznaczenia** między katalogami. Stos **will add** (bok lub dół) pokazuje, co zapisze Enter. Przy istniejącym configu odpala się w **trybie zarządzania** — zmiana nazwy/ścieżki/usuwanie istniejących workspace'ów i projektów (`n` przełącza na ekran skanowania, żeby dodać kolejne); to właśnie uruchamia domyślnie `orcan init` |
