...
| `orcan context tui` | TUI: skan katalogu-rodzica, multi-select do stosu **will add**, create/update workspace; opcjonalnie jeden branch → managed worktree na każde repo (`--sync`, `--yes`). Domyślny skan to **tylko dzieci** (`--depth 1`; `D` przełącza wnuki). **Enter** otwiera zwykły folder; **`u`** w górę; **`e`/`h`** skok; zaznaczenia zostają. **Tab** fokusuje stos (Space/`x` usuwa, Enter apply; **Esc** wraca na listę). Tagi: `in ws` / `other ws` / konflikty. Przy istniejącym configu — **tryb zarządzania** (`n` → skan); to uruchamia domyślnie `orcan init` |
