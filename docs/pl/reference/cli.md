...
| `orcan context tui` | TUI: skan katalogu i kompaktowa lista **will add**; opcjonalnie jeden branch → managed worktree na każde repo (`--sync`, `--yes`). Gdy nic nie wybrano, **Enter** otwiera wskazany folder; po zaznaczeniu projektów zatwierdza wybór. **Tab** otwiera pełny podgląd nowych, już podłączonych i konfliktowych pozycji. Ekran zarządzania ma zwijane grupy workspace (**←/→**). Domyślny skan pokazuje dzieci (`D` przełącza wnuki), a `h` przywołuje ostatnie wybory. |
